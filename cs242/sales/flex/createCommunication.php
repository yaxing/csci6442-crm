...
include_once 'Database.class.php';
	...
	$parent_account = $_POST['Parent_Account']; //"ConsultCity"; // dropdownlist
	$communication_with = $_POST['Account_Person']; //"Sun Kevin";// dropdownlist
	if (!isset($_COOKIE["basic"]['user_role'])){
		echo "1";//header('Location: login.html');//REDIRECTION PROBLEM
	}
	if ($_COOKIE["basic"]['user_role'] == "account_person" ){
		echo "2";//header('Location: login.html');//REDIRECTION PROBLEM
	}
	...
	if ($communication_id != 0)
		setcookie('communication[communication_id]', $communication_id, time()+60*60*24, "/", "");
