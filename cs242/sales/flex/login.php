...
if(true) {
	...
	die();*/
	$sql_cust = "SELECT account_person_id FROM account_person WHERE user_name='".$user_name."' AND user_pwd='".md5($user_pwd)."';";
	$sql_worker = "SELECT worker_id FROM worker WHERE user_name='".$user_name."' AND user_pwd='".md5($user_pwd)."';";
	$db->select($sql_cust);
	...
	if (empty($result_cust)){
		...
		echo "<br/> account_person_id: ".$user_id." <br/> person_role:".$user_role;
	}
