...
class SCRIPTSedited{
      ...
      // var $tablename = "Account_person";
       ...
       /**
        * The constructor initializes the db to database. Everytime
a request is
        * received by Zend AMF, an instance of the service class is created
and then the
       ...
       public function __construct() {
               ...
               $this->throwExceptionOnError($this->db);
       }

 
/*#region include
include_once '/Applications/MAMP/htdocs/DB2_DetailedTicketView-debug/config.php';
#endregion include
...
function customerCreateTicket() {
	...
	$applicant=$userId; 
	$applicant_location='Bucharest'; // ???
	$parent_account_of_Applicant=0; // ???
	...
	//execute statement
	$sql = "INSERT INTO Ticket VALUES ($parent_account_of_Applicant, $applicant, $applicant_location, '$customer_priority', '$ticket_description', '$ticket_summary', '$created_date', $created_by_worker);";
...
/**
  ...
  */ 
function workerViewTickets() {
	// variables you get from system
	$assigned_to_worker=$userId; 
	
	//execute statement
	$sql = "SELECT * FROM ticket WHERE ticked_id = ANY (SELECT parent_ticket FROM action WHERE assigned_to_worker = $assigned_to_worker);";
	$db->select($sql);
	$result = $db->fetchIndexArray();
}
...
/**
  ...
  */
function workerViewActionsForTicket() {
	// variables in form 
...
function workerCreateAction() {
	...
	//execute statement
	$sql = "INSERT INTO action VALUES ($parent_ticket, $assigned_to_worker, $assigned_to_skill, '$action_status', $action_created_by, '$action_prob_description', '$action_solution_description', 
...
/**
  ...
  */ 
function dispatcherCreateAction() {
	// variables in form 
	$parent_ticket=$_POST['parent_ticket'];	
	$action_prob_description=$_POST['action_prob_description']; 
	$action_solution_description=$_POST['action_solution_description'];
...
/**
  ...
  */ 
function dispatcherViewActions() {
	//execute statement
...
/**
  ...
  */ 
function dispatcherAssignActions() {
	// variables in form 
	$action_id=$_POST['action_id'];
